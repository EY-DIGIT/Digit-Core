...
public class RedirectController {
    ...
    @Value("${easebuzz.original.return.url.key}")
    private String returnUrlKey;

    @Value("${easebuzz.citizen.redirect.domain.name}")
    ...
    public String method(@RequestParam MultiValueMap<String, String> formData) {
    	...
    	else if(formData.get(PgConstants.PG_TXN_IN_LABEL_EASEBUZZ_DATA)!=null)
    		txnId = formData.get(PgConstants.PG_TXN_IN_LABEL_EASEBUZZ_DATA).get(0); 
    	else
    	{
    		txnId=returnURL.split(PgConstants.PG_TXN_IN_LABEL+"=")[1];
    	}
        ...
        /*
         * The EASEBUZZ integration is not allowing multiple schems or protocols (ex: HTTP, HTTPS)
